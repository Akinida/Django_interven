...
def student_view(request, studentId):
    try:
        ...
        allApp = Appointment.objects.filter(studentId=studentId).values()
        student_details = {
...
def mentorDisplay(request, MentorId):
    ...
    app = Appointment.objects.filter(mentorId=MentorId).values()
    context = {
        "Allreport":report,
        'mentorDetails':mentor,
        'allApp':app
...
def studentDisplay(request, studentId):
    ...
    app = Appointment.objects.filter(studentId=studentId)
    context = {
        "Allreport":report,
...
def submitAddStudent(request,adminId):
    if request.method == "POST":
        ...
        studcourse = request.POST["studentcourse"]

        # stuid = Student.objects.get(studentId=studId)
        mentor = Mentor.objects.get(mentorId=menId)

        addtoAdmin = Student(studentId=studId,studentName=studname,mentorId=mentor,studentPass=studpass,StudentAddress=studAddress,studentNo=studNo,Studentcourse=studcourse)
        addtoAdmin.save()
